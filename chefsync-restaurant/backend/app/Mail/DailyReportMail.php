...
class DailyReportMail extends Mailable
{
    ...
    private function buildHtml(): string
    {
        ...
        $body .= '<tr>';
        $body .= EmailLayoutHelper::statCard(number_format($r->total_revenue, 0) . ' &#8362;', 'הכנסות', '#22c55e');
        ...
        $body .= '<td style="width: 6px;"></td>';
        $body .= EmailLayoutHelper::statCard(number_format($r->cancelled_orders), 'ביטולים', '#ef4444');
        $body .= '</tr></table>';

        // אמצעי תשלום
        $body .= EmailLayoutHelper::sectionTitle('אמצעי תשלום');
        $body .= EmailLayoutHelper::infoBox(
            EmailLayoutHelper::infoRow('מזומן', number_format($r->cash_total, 0) . ' &#8362;')
                . EmailLayoutHelper::infoRow('אשראי', number_format($r->credit_total, 0) . ' &#8362;')
        );
