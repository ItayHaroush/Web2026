...
export default function MobileAddFab({ label, onClick, disabled, icon: Icon = FaPlus, positionClass = 'bottom-5 right-4' }) {
    return (
        <button
            ...
            onClick={onClick}
            disabled={disabled}
            className={`fixed z-[100] flex md:hidden items-center gap-2 rounded-full bg-brand-primary text-white shadow-xl shadow-brand-primary/35 px-4 py-3.5 font-black text-sm max-w-[min(92vw,20rem)] active:scale-[0.98] transition-transform disabled:opacity-40 disabled:pointer-events-none ${positionClass}`}
