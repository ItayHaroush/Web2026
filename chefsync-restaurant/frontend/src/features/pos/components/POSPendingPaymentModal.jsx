            ...
            <div className="bg-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden border border-slate-700 animate-in zoom-in-95 duration-300 flex flex-col relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto px-8 pb-8 space-y-3 custom-scrollbar">
                    {loading ? (
                        ...
                        orders.map(order => {
                            const srcKey = order.source || 'website';
                                ...
                                >
                                    <div className="p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                {order.delivery_method === 'delivery' && (
                                                    <span className="text-xs font-black px-2 py-1 rounded-lg bg-purple-500/15 text-purple-300">
                                                        משלוח
                                                    </span>
                                                )}
                                                {order.table_number && (
                                        ...
                                        {order.delivery_method === 'delivery' && (order.delivery_address || order.delivery_notes) && (
                                            <div className="mb-3 text-sm space-y-1.5 bg-purple-500/10 border border-purple-500/20 rounded-xl p-3">
                                                ...
                                                {order.delivery_notes && (
                                                    <p className="text-slate-500 text-xs mr-6">הערות: {order.delivery_notes}</p>
                                                )}
                                            </div>
                                        )}
